    ...
    <div className="flex items-center justify-center w-screen h-screen font-medium">
      <div className="flex flex-grow items-center justify-center bg-gray-900 h-full">
        <div className="max-w-full p-8 bg-gray-800 rounded-lg shadow-lg w-96 text-gray-200">
          ...
          {
            list.map(({ id, todo, completed }) => {
              return (
                <div className="group" key={id}>
                  <label className="flex items-center h-10 px-2 rounded cursor-pointer hover:bg-gray-900">
                    <input onChange={() => onChange(id, completed)} name="hidden-check" className="hidden peer" type="checkbox" defaultChecked={completed} />
                    ...
                    <span className="peer-checked:text-gray-500 peer-checked:line-through select-none mx-4 text-sm w-full" name="viewer">
                      {todo}
                    </span>
                    <button onClick={() => onClick(id)} className="hidden group-hover:block">
            ...
            <form onSubmit={onSubmit}>
              ...
              <div className="flex items-center w-full mt-2">
                {/* submit button[s] */}
                <button type="submit" className="h-8 px-2 text-sm font-medium rounded">
